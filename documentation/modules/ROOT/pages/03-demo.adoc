...
This namespace will be used to compose the expected fully qualified hostnames to be used in the AAP inventories, example given: `*server-a1.net-a*`.
...
The last piece to complete the {rhsi} setup is to install the generated site-bundles on the respective server used to reach third-party network `*net-a*` hosts.
...
The {aap} installation is expected to run in the "*aap*" namespace, using the {aap} Operator and an instance of the "Ansible Automation Platform" resource must be created and named "*my-aap*".
...
* A job templates (for Third-party Network A)
...
The inventory must store the following hostnames:
...
We need to create a job templates.

It will basically tie the Ansible project to be executed with the inventory that defines the target hosts and associate the credential to be used for each third-party network.
...
Run your job template and you should be able to see a successful result, as in the following example.
