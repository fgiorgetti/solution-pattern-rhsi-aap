...
[IMPORTANT]
====
The connectors extracted from the tar ball, are defined in two different files, named:
`./internal/net-a/30-connectors.yaml` and `./internal/net-b/30-connectors.yaml`. They are
defined using static IP addresses who must be updated, to work on your environment. +
Make sure to update them, otherwise {rhsi} won't be able to reach the target servers.
====
